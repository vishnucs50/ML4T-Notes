...
class decision_tree:
    ...
    def initial_create_tree(self):
        self.decision_tree = self.construct_tree(self.data_x, self.data_y)
    ...
    def construct_tree(self, data_x, data_y):
        if data_x.shape[0] == 1:
            return np.array([ [-1, data_y[0], np.nan, np.nan]])
        if np.all(data_y == data_y[0]):
            return np.array([[-1, np.mean(data_y), np.nan, np.nan]])
        if data_x.shape[0] == 0:  # Handle empty data
            return np.array([])

        # if neither of the base cases work, continue
        best_feature = self.best_feature(data_x, data_y)
        split_value = np.median(data_x[:, best_feature])

        #left and right values
        left_half = data_x[:, best_feature] <= split_value
        right_half = data_x[:, best_feature] > split_value

        # left and right trees
        left_tree = self.construct_tree(data_x[left_half], data_y[left_half])
        right_tree = self.construct_tree(data_x[right_half], data_y[right_half])

        root_node = np.array([ [best_feature, split_value, 1, left_tree.shape[0] + 1]])
    ...
    def prediction(self, data):
        ...
        def traverse_tree(points, node_index):
            ...
            if points[feature_index] <= split_value:
        ...
        def starting_point(point):
            return traverse_tree(point, 0)

        predictions = np.apply_along_axis(starting_point, 1, data)
...
if __name__ == '__main__':
    ...
    tester = decision_tree(x_train, y_train)
    print(tester.initial_create_tree())
    ...
    print(tester.prediction(x_test))